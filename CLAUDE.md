...
| Config variables | `dataxrayUrl`, `dataxrayAuthToken` (sentinel defaults) | `dataxrayConnectionName` (default `data-xray`), `dataxrayPageSize` (50, search/rerun), `dataxrayUrl` (links only, optional) |
...
**Page sizing**: `edgeExcludedFields()` drops phrases, raw text, computed/entitlement fields → 50 rows ≈ 60–80 KB. `dataxrayPageSize` is clamped to 1–200.

**Testing recipe**: deploy with `--variant edge`; on ohalo.collibra.com the Edge site `ohalo-edge-integration` already has an HTTP connection **`ohalo`** (Basic auth, user `life_sciences` @ demo.dataxray.io — org unit 8, so its labels differ from the ones synced by Alistair's user; read-only annotators such as *Email (Regex)* are visible to both). Start workflows via `POST /rest/2.0/workflowInstances` passing `formProperties.dataxrayConnectionName` = `ohalo`; poll `GET /rest/2.0/workflowTasks` for the instance's task (async: the results task appears ~7 s later); `GET /rest/2.0/workflowTasks/{id}/taskFormData` exists but exposes no variables for HTML forms, so verify via the query asset (attributes, relations) and the Files domain. Verified 2026-09-15: search 64 hits → import 64 file assets in 2 pages (~7 s) with correct attributes and classification links; rerun kept 64 `returns` relations; wrong connection name → instance in error. Don't run the classification sync through `ohalo` on dev: it would retire every classification of the other org unit.
