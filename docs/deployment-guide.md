...
| **Search Data X-Ray** | Builds a Data X-Ray search query from Collibra classifications (every selected label, extractor and annotator must match — criteria are always AND-ed; the optional annotated text may appear in any of the annotators chosen for it), previews the matching files, and offers to **import all results as file assets** (with an optional nightly keep-in-sync flag). | From the **+ Create** menu, on demand. |
